...
function pos() { return String(genId()).padStart(5, '0'); }
function childOf(g) { return { guid: g, position: pos() }; }
function pageOf(s, l) { return { guid: { sessionID: s, localID: l }, position: pos() }; }
...
function card(doc, nm, cost, desc, dmg, _blk, typ, x, y, parentGuid) {
  const cw = 160, ch = 220;
  ...
  const pg = () => childOf(parentGuid);

  rect(doc, `${nm} sh`, x + 3, y + 3, cw, ch, paint('#000000', 0.4), 10, pg());
  rect(doc, `${nm} bg`, x, y, cw, ch, paint(cardBg), 10, pg());

  n(doc, {
    type: 'RECTANGLE', name: `${nm} acc`, parentIndex: pg(),
    size: { x: cw, y: 4 },
    transform: { m00: 1, m01: 0, m02: x, m10: 0, m11: 1, m12: y },
  ...
  rect(doc, `${nm} cost`, x + 8, y + 8, 28, 28, paint('#ffcc00'), 14, pg());
  txt(doc, String(cost), x + 16, y + 11, 15, paint('#000000'), pg(), true);
  txt(doc, nm, x + 44, y + 12, 13, paint('#ffffff'), pg(), true);
  txt(doc, typ.toUpperCase(), x + 44, y + 30, 9, paint(accent), pg());

  rect(doc, `${nm} art`, x + 10, y + 48, cw - 20, 90, paint('#0d0d1a'), 6, pg());
  if (dmg) {
    rect(doc, `${nm} dmg`, x + 55, y + 75, 50, 36, paint(accent, 0.3), 6, pg());
    txt(doc, dmg, x + 60, y + 78, 22, paint('#ffffff'), pg(), true);
    if (!dmg.includes('x')) {
      txt(doc, 'DMG', x + 62, y + 102, 8, paint(accent, 0.7), pg());
    }
  }
  txt(doc, desc, x + 10, y + 148, 9, paint('#8888aa'), pg());

  n(doc, {
    type: 'RECTANGLE', name: `${nm} bot`, parentIndex: pg(),
    size: { x: cw, y: 2 },
...
card(doc, 'Slice', 1, 'Deal 8 damage', '8', null, 'Attack', 30, 350, ch);
card(doc, 'Double Slice', 2, 'Deal 5 dmg x2', '5x2', null, 'Attack', 205, 350, ch);
card(doc, 'Defend', 1, 'Gain 5 Block', null, '5', 'Skill', 380, 350, ch);
card(doc, 'Neural Surge', 3, 'Deal 14 damage', '14', null, 'Attack', 555, 350, ch);
card(doc, 'Overclock', 1, 'Draw 2 cards', null, null, 'Skill', 730, 350, ch);
